...
import { TelegramTask, Token, UserData, Proof, VerificationMethod } from '../../types';
import TokenIcon from '../icons/TokenIcon';
import TelegramIcon from '../icons/TelegramIcon';
import ConfirmationModal from '../ConfirmationModal';

const TelegramTaskManager: React.FC = () => {
    const { state, addTelegramTask, updateTelegramTask, removeTelegramTask } = useContext(AppContext);
    ...
    const handleEdit = (task: TelegramTask) => {
        setTitle(task.title);
        setDescription(task.description);
        setTelegramUrl(task.telegramUrl);
    ...
    const handleCancelEdit = () => {
        setTitle(''); setDescription(''); setTelegramUrl(''); setReward('');
    ...
    const handleSubmit = async () => {
        ...
        try {
            const taskData: TelegramTask = {
                id: editingId || new Date().getTime().toString(),
                title, description, telegramUrl, reward: parseFloat(reward), rewardToken,
                ...
                claimCount: editingId ? (state.telegramTasks.find(t => t.id === editingId)?.claimCount || 0) : 0,
                dailyClaimCount: editingId ? (state.telegramTasks.find(t => t.id === editingId)?.dailyClaimCount || 0) : 0
            };
            if (editingId) await updateTelegramTask(taskData);
            else await addTelegramTask(taskData);
            handleCancelEdit();
        } catch (err: any) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={() => { removeTelegramTask(itemToDelete!); setItemToDelete(null); }} title="Remove Telegram Task" message="Are you sure?" />
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl border border-gray-200 dark:border-blue-900/30">
                <h2 className="text-2xl font-bold mb-4 text-blue-400 border-b-2 border-blue-400 pb-2 flex items-center space-x-2"><TelegramIcon className="w-7 h-7" /><span>Telegram Manager</span></h2>
                <div className="space-y-4">
                    <input type="text" placeholder="Channel Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" />
                    ...
                    <input type="text" placeholder="Invite Link" value={telegramUrl} onChange={(e) => setTelegramUrl(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" />
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-lg transition shadow-lg shadow-blue-900/20">{isSubmitting ? 'Saving...' : (editingId ? 'Update Task' : 'Add Telegram Task')}</button>
                </div>
            </div>
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl lg:max-h-[calc(100vh-15rem)]">
                <h3 className="text-xl font-bold mb-4 text-gray-300 border-b border-gray-600 pb-2">All Telegram Tasks</h3>
                <input 
                    type="text" 
                ...
                <div className="space-y-3 overflow-y-auto h-full pr-2">
                    {state.telegramTasks.filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase())).map(task => (

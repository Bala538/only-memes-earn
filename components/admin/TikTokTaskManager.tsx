...
import { TikTokTask, Token, UserData, Proof, VerificationMethod } from '../../types';
import TokenIcon from '../icons/TokenIcon';
import TikTokIcon from '../icons/TikTokIcon';
import ConfirmationModal from '../ConfirmationModal';

const TikTokTaskManager: React.FC = () => {
    const { state, addTikTokTask, updateTikTokTask, removeTikTokTask } = useContext(AppContext);
    ...
    const handleEdit = (task: TikTokTask) => {
        setTitle(task.title);
        setDescription(task.description);
        setTikTokUrl(task.tiktokUrl);
    ...
    const handleCancelEdit = () => {
        setTitle(''); setDescription(''); setTikTokUrl(''); setReward('');
    ...
    const handleSubmit = async () => {
        ...
        try {
            const taskData: TikTokTask = {
                id: editingId || new Date().getTime().toString(),
                title, description, tiktokUrl, reward: parseFloat(reward), rewardToken,
                ...
                claimCount: editingId ? (state.tiktokTasks.find(t => t.id === editingId)?.claimCount || 0) : 0,
                dailyClaimCount: editingId ? (state.tiktokTasks.find(t => t.id === editingId)?.dailyClaimCount || 0) : 0
            };
            if (editingId) await updateTikTokTask(taskData);
            else await addTikTokTask(taskData);
            handleCancelEdit();
        } catch (err: any) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={() => { removeTikTokTask(itemToDelete!); setItemToDelete(null); }} title="Remove TikTok Task" message="Are you sure?" />
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700">
                <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white border-b-2 border-gray-400 pb-2 flex items-center space-x-2"><TikTokIcon className="w-7 h-7" /><span>TikTok Manager</span></h2>
                <div className="space-y-4">
                    <input type="text" placeholder="Task Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-gray-500" />
                    <textarea placeholder="Task Description (Optional)" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-gray-500" rows={2} />
                    <input type="text" placeholder="TikTok Video/Profile URL" value={tiktokUrl} onChange={(e) => setTikTokUrl(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-gray-500" />
                    ...
                    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-3">
                        ...
                        <select value={verificationMethod} onChange={(e) => setVerificationMethod(e.target.value as VerificationMethod)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-gray-500">
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-gray-600 hover:bg-gray-500 text-white font-bold py-4 rounded-lg transition shadow-lg shadow-gray-900/20">{isSubmitting ? 'Saving...' : (editingId ? 'Update Task' : 'Add TikTok Task')}</button>
                </div>
            </div>
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl lg:max-h-[calc(100vh-15rem)]">
                <h3 className="text-xl font-bold mb-4 text-gray-300 border-b border-gray-600 pb-2">All TikTok Tasks</h3>
                <input 
                    type="text" 
                ...
                <div className="space-y-3 overflow-y-auto h-full pr-2">
                    {state.tiktokTasks.filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase())).map(task => (
                        <div key={task.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg flex justify-between items-center border border-gray-200 dark:border-gray-700">
                            <div className="min-w-0">
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    <span className="text-[10px] bg-gray-700 text-gray-300 px-1.5 py-0.5 rounded uppercase font-bold">{task.reward} {state.tokenConfigs[task.rewardToken]?.name || task.rewardToken}</span>
...
export default TikTokTaskManager;

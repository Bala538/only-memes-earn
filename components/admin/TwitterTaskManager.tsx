...
import { TwitterTask, Token, UserData, Proof, VerificationMethod } from '../../types';
import TokenIcon from '../icons/TokenIcon';
import TwitterIcon from '../icons/TwitterIcon';
import ConfirmationModal from '../ConfirmationModal';

const TwitterTaskManager: React.FC = () => {
    const { state, addTwitterTask, updateTwitterTask, removeTwitterTask } = useContext(AppContext);
    ...
    const handleEdit = (task: TwitterTask) => {
        setTitle(task.title);
        setDescription(task.description);
        setTwitterUrl(task.twitterUrl);
    ...
    const handleCancelEdit = () => {
        setTitle(''); setDescription(''); setTwitterUrl(''); setReward('');
    ...
    const handleSubmit = async () => {
        ...
        try {
            const taskData: TwitterTask = {
                id: editingId || new Date().getTime().toString(),
                title, description, twitterUrl, reward: parseFloat(reward), rewardToken,
                ...
                claimCount: editingId ? (state.twitterTasks.find(t => t.id === editingId)?.claimCount || 0) : 0,
                dailyClaimCount: editingId ? (state.twitterTasks.find(t => t.id === editingId)?.dailyClaimCount || 0) : 0
            };
            if (editingId) await updateTwitterTask(taskData);
            else await addTwitterTask(taskData);
            handleCancelEdit();
        } catch (err: any) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={() => { removeTwitterTask(itemToDelete!); setItemToDelete(null); }} title="Remove Twitter Task" message="Are you sure?" />
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl border border-gray-200 dark:border-sky-900/30">
                <h2 className="text-2xl font-bold mb-4 text-sky-400 border-b-2 border-sky-400 pb-2 flex items-center space-x-2"><TwitterIcon className="w-7 h-7" /><span>Twitter Manager</span></h2>
                <div className="space-y-4">
                    <input type="text" placeholder="Task Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-sky-500" />
                    <textarea placeholder="Task Description (Optional)" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-sky-500" rows={2} />
                    <input type="text" placeholder="X / Twitter URL" value={twitterUrl} onChange={(e) => setTwitterUrl(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-sky-500" />
                    ...
                    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-3">
                        ...
                        <select value={verificationMethod} onChange={(e) => setVerificationMethod(e.target.value as VerificationMethod)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-sky-500">
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-sky-500 hover:bg-sky-600 text-white font-bold py-4 rounded-lg transition shadow-lg shadow-sky-900/20">{isSubmitting ? 'Saving...' : (editingId ? 'Update Task' : 'Add Twitter Task')}</button>
                </div>
            </div>
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl lg:max-h-[calc(100vh-15rem)]">
                <h3 className="text-xl font-bold mb-4 text-gray-300 border-b border-gray-600 pb-2">All Twitter Tasks</h3>
                <input 
                    type="text" 
                ...
                <div className="space-y-3 overflow-y-auto h-full pr-2">
                    {state.twitterTasks.filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase())).map(task => (
                        <div key={task.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg flex justify-between items-center border border-gray-200 dark:border-gray-700">
                            <div className="min-w-0">
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    <span className="text-[10px] bg-sky-900/50 text-sky-300 px-1.5 py-0.5 rounded uppercase font-bold">{task.reward} {state.tokenConfigs[task.rewardToken]?.name || task.rewardToken}</span>
...
export default TwitterTaskManager;

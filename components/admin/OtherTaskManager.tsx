...
import { OtherTask, Token, VerificationMethod } from '../../types';
import ConfirmationModal from '../ConfirmationModal';

const OtherTaskManager: React.FC = () => {
    const { state, addOtherTask, updateOtherTask, removeOtherTask } = useContext(AppContext);
    ...
    const handleEdit = (task: OtherTask) => {
        setTitle(task.title);
    ...
    const handleCancelEdit = () => {
        setTitle(''); setDescription(''); setTaskUrl(''); setReward('');
    ...
    const handleSubmit = async () => {
        ...
        try {
            const taskData: OtherTask = {
                id: editingId || new Date().getTime().toString(),
                title, description, taskUrl, reward: parseFloat(reward), rewardToken,
                ...
                claimCount: editingId ? (state.otherTasks.find(t => t.id === editingId)?.claimCount || 0) : 0,
                dailyClaimCount: editingId ? (state.otherTasks.find(t => t.id === editingId)?.dailyClaimCount || 0) : 0
            };
            if (editingId) await updateOtherTask(taskData);
            else await addOtherTask(taskData);
            handleCancelEdit();
        } catch (err: any) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={() => { removeOtherTask(itemToDelete!); setItemToDelete(null); }} title="Remove Other Task" message="Are you sure?" />
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700">
                <h2 className="text-2xl font-bold mb-4 text-gray-300 border-b-2 border-gray-600 pb-2 flex items-center space-x-2">
                ...
                <div className="space-y-4">
                    ...
                    <input type="text" placeholder="Task URL" value={taskUrl} onChange={(e) => setTaskUrl(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-gray-500" />
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-gray-600 hover:bg-gray-500 text-white font-bold py-4 rounded-lg transition shadow-lg shadow-gray-900/20">{isSubmitting ? 'Saving...' : (editingId ? 'Update Task' : 'Add Other Task')}</button>
                </div>
            </div>
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl lg:max-h-[calc(100vh-15rem)]">
                <h3 className="text-xl font-bold mb-4 text-gray-300 border-b border-gray-600 pb-2">All Other Tasks</h3>
                <input 
                    type="text" 
                ...
                <div className="space-y-3 overflow-y-auto h-full pr-2">
                    {state.otherTasks.filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase())).map(task => (
                        <div key={task.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg flex justify-between items-center border border-gray-200 dark:border-gray-700">
                            <div className="min-w-0">
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    <span className="text-[10px] bg-gray-900/50 text-gray-300 px-1.5 py-0.5 rounded uppercase font-bold">{task.reward} {state.tokenConfigs[task.rewardToken]?.name || task.rewardToken}</span>
                                    ...
                                    {task.verificationMethod === 'code' && <span className="text-[10px] bg-gray-900/50 text-gray-300 px-1.5 py-0.5 rounded">Auto</span>}
...
export default OtherTaskManager;

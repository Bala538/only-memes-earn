...
import { FacebookTask, Token, UserData, Proof, VerificationMethod } from '../../types';
import TokenIcon from '../icons/TokenIcon';
import FacebookIcon from '../icons/FacebookIcon';
import ConfirmationModal from '../ConfirmationModal';

const FacebookTaskManager: React.FC = () => {
    const { state, addFacebookTask, updateFacebookTask, removeFacebookTask } = useContext(AppContext);
    ...
    const handleEdit = (task: FacebookTask) => {
        setTitle(task.title);
        setDescription(task.description);
        setFacebookUrl(task.facebookUrl);
    ...
    const handleCancelEdit = () => {
        setTitle(''); setDescription(''); setFacebookUrl(''); setReward('');
    ...
    const handleSubmit = async () => {
        ...
        try {
            const taskData: FacebookTask = {
                id: editingId || new Date().getTime().toString(),
                title, description, facebookUrl, reward: parseFloat(reward), rewardToken,
                ...
                claimCount: editingId ? (state.facebookTasks.find(t => t.id === editingId)?.claimCount || 0) : 0,
                dailyClaimCount: editingId ? (state.facebookTasks.find(t => t.id === editingId)?.dailyClaimCount || 0) : 0
            };
            if (editingId) await updateFacebookTask(taskData);
            else await addFacebookTask(taskData);
            handleCancelEdit();
        } catch (err: any) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={() => { removeFacebookTask(itemToDelete!); setItemToDelete(null); }} title="Remove Facebook Task" message="Are you sure?" />
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl border border-gray-200 dark:border-blue-900/30">
                <h2 className="text-2xl font-bold mb-4 text-blue-600 border-b-2 border-blue-600 pb-2 flex items-center space-x-2"><FacebookIcon className="w-7 h-7" /><span>Facebook Manager</span></h2>
                <div className="space-y-4">
                    <input type="text" placeholder="Task Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" />
                    <textarea placeholder="Task Description (Optional)" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" rows={2} />
                    <input type="text" placeholder="Facebook Profile/Page URL" value={facebookUrl} onChange={(e) => setFacebookUrl(e.target.value)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded-md border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" />
                    ...
                    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-3">
                        ...
                        <select value={verificationMethod} onChange={(e) => setVerificationMethod(e.target.value as VerificationMethod)} className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-white p-3 rounded border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500">
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-lg transition shadow-lg shadow-blue-900/20">{isSubmitting ? 'Saving...' : (editingId ? 'Update Task' : 'Add Facebook Task')}</button>
                </div>
            </div>
            <div className="bg-white dark:bg-[#161B22] p-6 rounded-lg shadow-xl lg:max-h-[calc(100vh-15rem)]">
                <h3 className="text-xl font-bold mb-4 text-gray-300 border-b border-gray-600 pb-2">All Facebook Tasks</h3>
                <input 
                    type="text" 
                ...
                <div className="space-y-3 overflow-y-auto h-full pr-2">
                    {state.facebookTasks.filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase())).map(task => (
                        <div key={task.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg flex justify-between items-center border border-gray-200 dark:border-gray-700">
                            <div className="min-w-0">
                                ...
                                <span className="text-[10px] bg-blue-900/50 text-blue-300 px-1.5 py-0.5 rounded uppercase font-bold">{task.reward} {state.tokenConfigs[task.rewardToken]?.name || task.rewardToken}</span>
...
export default FacebookTaskManager;
